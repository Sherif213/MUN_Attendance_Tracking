...
import { TextField, Button, Container, Typography, Grid, Card, CardContent, Divider } from "@mui/material";
...
const AttendanceForm = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setDateTime(new Date().toLocaleString("en-GB", { timeZone: "Europe/Istanbul" }));
    }, 1000);
  ...
  if (submitted) {
    return (
      <Container maxWidth="md" sx={{ display: "flex", flexDirection: "column", alignItems: "center", mt: 5 }}>
        <Card sx={{ p: 3, boxShadow: 3, borderRadius: 3, width: "100%", maxWidth: 600, bgcolor: "#ffffff" }}>
          <CardContent>
  ...
  return (
    <Container maxWidth="md" sx={{ display: "flex", flexDirection: "column", alignItems: "center", mt: 5 }}>
      <Card sx={{ p: 3, boxShadow: 3, borderRadius: 3, width: "100%", maxWidth: 600, bgcolor: "#ffffff" }}>
        <CardContent>
          <Typography variant="h6" gutterBottom align="center" sx={{ fontWeight: "bold", color: "#555" }}>
            {dateTime}
          </Typography>
            ...
            {Object.keys(attendance).map((session, index) => (
              <Grid item xs={4} key={index} sx={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Typography sx={{ display: "flex", alignItems: "center", fontWeight: "bold" }}>
                  ...
                  {attendance[session] ? <CheckCircle color="success" sx={{ ml: 1 }} /> : <Cancel color="error" sx={{ ml: 1 }} />}
                </Typography>
              </Grid>
            ))}
          </Grid>
